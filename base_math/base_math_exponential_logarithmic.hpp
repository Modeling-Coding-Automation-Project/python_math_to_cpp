...
inline double sqrt_extraction_double(const double &value) {
  ...
  for (i = 54; i >= (-2 * EXTRACTION_DOUBLE_REPEAT_NUMBER - 2) *
                        (EXTRACTION_DOUBLE_REPEAT_NUMBER < -1);
       i -= 2) {
  ...
  for (i = 0; i <= EXTRACTION_DOUBLE_REPEAT_NUMBER; i++) {
  ...
  Temp += (((a + ((unsigned long long)1 & a)) >> 1)
           << (26 - EXTRACTION_DOUBLE_REPEAT_NUMBER));

  std::memcpy(&result, &Temp, 8);
